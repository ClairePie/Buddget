...
def connection():
    ...
    #Home
    #home = Button(window2, text="Accueil", font=("Noto Sans CJK KR", 17, "bold"), bg="orange", fg='white', activebackground="gray", activeforeground="white", bd=0, command = accueil(window, height, width))
    #home.place(x=35, y=60, width=120, height = 22)
    ...
    #Valider
    valider = Button(window2, text="Valider", font=("Noto Sans CJK KR", 15, "bold"), bd=0, bg="orange", fg="white", activebackground="white", activeforeground="orange", command=verify)
...
def compteCreation():
    ...
    #Valider
    valider=Button(window3, text="Valider", fg="white", bg = "orange", activebackground="white", activeforeground="orange", font=("Noto Sans CJK KR", 15, "bold"), command=creationUser, bd=0)
...
def open_secondary_window(identifiant):
    ...
    # Sauvegarde excel
    def save_to_excel(expenses, benefits, previousSolde, description):
    ...
    def revenue():
        ...
        if addi >= 0:    
                ...
                save_to_excel(0, addi, previousSolde, description)  # Save revenue to Excel
    ...
    def depense():
        ...
        try:  
            ...
            fina['text'] = "Votre solde est maintenant : {} €".format(previousSolde)
            save_to_excel(minus, 0, previousSolde, description)  # Save expense to Excel
    ...
    def valider():
        ...
        try:
                ...
                solde_entry.delete(0, END)
                fina['text'] = "Votre solde est maintenant : {} €".format(previousSolde)
                save_to_excel(0, 0, previousSolde, legende)  # Save new total to Excel
    ...
    def delete_selected(identifiant):
        global previousSolde
    ...
    def edit_selected():
        ...
        if selected_index:
            ...
            new_value_button= Button(compte, text="Valider", command=lambda: update_selected(selected_index, new_value_entry.get()), bd=0, bg="sandy brown", fg="white", font=("verdana", 10, "bold"))
            new_value_button.place(x=width/3+200, y=height-75)
            old_value = listbox.get(selected_index)
            new_value_entry.insert(0, old_value)
    
    def update_selected(index, new_value):
        global previousSolde
        index = index[0]
    ...
    #Bouton
    less = Button(compte, text=" - ",command= depense,font=("verdana", 12, "bold"), width=5, bd=0, bg="orange", fg="white")
    ...
    #Bouton
    add = Button(compte, text=" + ",command= revenue, font=("verdana", 12, "bold"), width = 5, bd=0, bg="orange", fg="white")
    ...
    solde_1.place(x=width/12, y=545)
        ...
        #Bouton
    valid2 = Button(compte, text="Valider", command=valider, font=("verdana", 11, "bold"), width = 7, bg='orange', bd=0, fg='white')
    ...
    # Annuel
    plot_monthly = Button(compte, text="Initialiser", command=graphiqueAnnuel, font=("Verdana", 10), bg="white", fg="sandy brown", bd=0)
    plot_monthly.place(x=width-120, y=350)

    # Mensuel
    plot_yearly = Button(compte, text="Initialiser", command=graphiqueMensuel, font=("verdana", 10), bg="white", fg="sandy brown", bd=0)
    ...
    delete_button = Button(compte, text="Supprimer la sélection",bd=0, bg='white', fg='sandy brown', font=('verdana', 10, "bold"), command=lambda:delete_selected(identifiant))
    delete_button.place(x=width/3+80,  y= 620)  
    edit_button = Button(compte, text="Modifier",bd=0, bg="white", fg="sandy brown", font=("verdana", 10), command=edit_selected)
